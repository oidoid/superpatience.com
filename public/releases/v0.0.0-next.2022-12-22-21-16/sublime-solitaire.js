...
function assertNonNull1(val, msg) {
    assert(val != null, msg ?? 'Expected nonnullish value.');
}
const __default = JSON.parse(`{
  ...
  "perInstance": [
    ...
    { "name": "iLayer", "type": "UNSIGNED_SHORT", "len": 1 },
    { "name": "iMoreBits", "type": "UNSIGNED_SHORT", "len": 1 }
  ]
}
`);
function Synth() {
...
function updateEnt(set, update) {
    ...
    I16Box.sizeTo(sprite.bounds, I16(followCam.fill == 'X' || followCam.fill == 'XY' ? I16Box.width(update.camBounds) - pad.x * 2 : I16Box.width(sprite.bounds)), I16(followCam.fill == 'Y' || followCam.fill == 'XY' ? I16Box.height(update.camBounds) - pad.y * 2 : I16Box.height(sprite.bounds)));
...
var Input;
(function(Input) {
    function activeTriggered(input) {
        return input?.active == true && input.timer == 0;
    }
    Input.activeTriggered = activeTriggered;
    function activeLong(input) {
        return input?.active == true && input.timer > 500;
    }
    Input.activeLong = activeLong;
    function inactiveTriggered(input) {
        return input == null || !input.active && input.timer == 0;
    }
    Input.inactiveTriggered = inactiveTriggered;
...
(function(DataTypeSize) {
    DataTypeSize[DataTypeSize["BYTE"] = 1] = "BYTE";
    DataTypeSize[DataTypeSize["UNSIGNED_BYTE"] = 1] = "UNSIGNED_BYTE";
    DataTypeSize[DataTypeSize["SHORT"] = 2] = "SHORT";
    DataTypeSize[DataTypeSize["UNSIGNED_SHORT"] = 2] = "UNSIGNED_SHORT";
    DataTypeSize[DataTypeSize["INT"] = 4] = "INT";
    DataTypeSize[DataTypeSize["UNSIGNED_INT"] = 4] = "UNSIGNED_INT";
    DataTypeSize[DataTypeSize["FLOAT"] = 4] = "FLOAT";
})(DataTypeSize || (DataTypeSize = {}));
var ShaderLayoutParser;
(function(ShaderLayoutParser) {
    ShaderLayoutParser.parse = (config)=>Immutable({
...
function parseAttributes(divisor, configs) {
    ...
    const maxDataTypeSize = attributes.map(({ type  })=>DataTypeSize[type]).reduce((max, val)=>Math.max(max, val), 0);
...
const reduceAttributeVariable = (attributes, { type , name , len  }, index)=>{
...
const nextAttributeOffset = (attr)=>attr.offset + DataTypeSize[attr.type] * attr.len;
...
var LayerSuborder;
(function(LayerSuborder) {
    LayerSuborder.values = Immutable(new Set([
        'End',
        'Start'
    ]));
...
const LayerSuborderFlag = U16(0b1000_0000);
const LayerSuborderMask = U16(LayerSuborderFlag);
const LayerSuborderFlagStart = U16(LayerSuborderMask & LayerSuborderFlag);
const LayerSuborderFlagEnd = U16(LayerSuborderMask & ~LayerSuborderFlag);
var JSONStorage;
...
const __default1 = `#version 300 es
...
const __default2 = `#version 300 es
#pragma debug(${GL.debug ? 'on' : 'off'})
#pragma optimize(${GL.debug ? 'off' : 'on'})
...
function Renderer(canvas, atlas, layout, atlasMeta) {
    ...
    const [r, g, b, a] = Color.intToFloats(U32(0xe6e6df_ff));
    ...
    const program = GL.loadProgram(gl, __default1, __default2);
...
function Sprite(film, layer, props) {
    ...
    return {
        ...
        wrapSuborderLayer: serializeWrapSuborderLayer(I4XY(wrapX, wrapY), layerSuborder, layer),
...
(function(ECS) {
    ...
    function get(self, ent, ...types) {
        ...
        for (const type of types){
            ...
            assertNonNull1(component, `Ent ${ent} missing ${String(type)} component.`);
...
function updateEnt2(set, update) {
    ...
    if (update.inputs.point?.pointerType == 'Pen' || update.inputs.point?.pointerType == 'Touch') Sprite.setLayer(sprite, Layer.Bottom);
    else Sprite.setLayer(sprite, Layer.Cursor);
    if (update.inputs.pick?.active == true && (update.inputs.pick.buttons & Button.Primary) == Button.Primary) {
...
(function(InputPoller) {
    ...
    InputPoller.make = make;
    function register(self, window1, op) {
        const fn = `${op}EventListener`;
        const types = [
            'contextmenu',
            'pointerup',
        ...
        for (const type of types){
            window1[fn](type, self.onEvent, {
                capture: true,
                passive: type != 'contextmenu' && type != 'pointerdown'
            });
        }
    }
    InputPoller.register = register;
    function update(self, time, clientViewportWH, cam) {
        self.clientViewportWH = clientViewportWH;
        self.cam = cam;
        InputState.update(self.inputs, time);
    }
    InputPoller.update = update;
...
function eventToPoint(self, ev, received) {
...
function eventToPick(self, ev, received) {
    if (ev.type == 'pointercancel') return undefined;
    const { pick  } = self.inputs;
    const active = ev.type == 'pointerdown' || ev.type == 'pointermove' && pick?.active == true;
...
function parsePointerType(type) {
    const pointerType = Str1.capitalize(type);
    assert1(pointerType == 'Mouse' || pointerType == 'Pen' || pointerType == 'Touch');
    return pointerType;
}
(function(Renderer) {
...
(function(Sprite) {
    function toString(self) {
        ...
        return `Sprite {id=${self.animator.film.id} box=${I16Box.toString(self.bounds)} layer=${wsl.layer} suborder=${wsl.suborder} flip=${morebits.flip || 'None'} wrap=${I4XY.toString(wsl.wrap)}}`;
    }
    Sprite.toString = toString;
    function intersectsBounds(self, xyOrBoxOrSprite) {
        if ('bounds' in xyOrBoxOrSprite) {
            return I16Box.intersects(self.bounds, xyOrBoxOrSprite.bounds);
        }
        return I16Box.intersects(self.bounds, xyOrBoxOrSprite);
    }
    Sprite.intersectsBounds = intersectsBounds;
    function getCel(self, time) {
        return self.animator.cel(time);
    }
    Sprite.getCel = getCel;
    function intersectsSprite(self, sprite, time) {
        ...
        for (const slice of cel.slices){
            ...
            if (intersects(self, box1, time)) return true;
        }
        return false;
    }
    Sprite.intersectsSprite = intersectsSprite;
    function intersects(self, xyOrBox, time) {
        if (!intersectsBounds(self, xyOrBox)) return false;
    ...
    Sprite.setLayer = setLayer;
    function moveTo(self, xy) {
        I16Box.moveTo(self.bounds, xy);
        return self;
    }
    Sprite.moveTo = moveTo;
    function reset(self, start, film) {
        self.animator.reset(start, film);
    }
    Sprite.reset = reset;
    function compareDepth(lhs, rhs) {
        const { suborder: lhsSuborder , layer: lhsLayer  } = parseWrapSuborderLayer(lhs.wrapSuborderLayer);
        const { suborder: rhsSuborder , layer: rhsLayer  } = parseWrapSuborderLayer(rhs.wrapSuborderLayer);
        return lhsLayer == rhsLayer ? rhs.bounds[Str.uncapitalize(rhsSuborder)].y - lhs.bounds[Str.uncapitalize(lhsSuborder)].y : lhsLayer - rhsLayer;
    }
    Sprite.compareDepth = compareDepth;
    function parseWrapSuborderLayer(wrapSuborderLayer) {
    ...
    Sprite.parseWrapSuborderLayer = parseWrapSuborderLayer;
})(Sprite || (Sprite = {}));
function parseMoreBits(moreBits) {
    const flipX = (NumUtil.ushift(moreBits, 1) & 1) == 1;
    const flipY = (NumUtil.ushift(moreBits, 0) & 1) == 1;
    return {
        flip: `${flipX ? 'X' : ''}${flipY ? 'Y' : ''}`
    };
}
function serializeWrapSuborderLayer(wrapXY, layerSuborder, layer) {
    const suborder = layerSuborder == 'End' ? LayerSuborderFlagEnd : LayerSuborderFlagStart;
    const wrap = NumUtil.lshift(wrapXY.x & 0xf, 12) | NumUtil.lshift(wrapXY.y & 0xf, 8);
    return U16(wrap | suborder | layer);
}
function serializeMoreBits(flip) {
...
(function(Assets) {
    async function load() {
        ...
        const shaderLayout = ShaderLayoutParser.parse(__default);
        return {
            atlas,
            atlasMeta: __default3,
            shaderLayout
        };
    }
    Assets.load = load;
...
Immutable(Inverse1(SublimeLayer));
const boardX = 2 * 8;
const hiddenY = -1024;
function setSpritePositionsForLayout(ecs, filmByID, solitaire, time) {
    for (const [indexX, column] of solitaire.tableau.entries()){
        for (const [indexY, card] of column.entries()){
            ...
            Sprite.setLayer(components.sprite, SublimeLayer[card.direction == 'Up' ? 'CardUp' : 'CardDown']);
    ...
    for (const pillar of solitaire.foundation){
        for (const [index, card1] of pillar.entries()){
            ...
            Sprite.moveTo(components1.sprite, xy1);
            const animID = index == pillar.length - 1 ? getCardFilmID(card1) : 'CardDown';
            Sprite.reset(components1.sprite, time, filmByID[animID]);
            Sprite.setLayer(components1.sprite, SublimeLayer[animID == 'CardDown' ? 'CardDown' : 'CardUp']);
        }
    }
    for (const [index1, card2] of solitaire.stock.entries()){
        const components2 = ecs.componentsByRef.get(card2);
        Sprite.moveTo(components2.sprite, getStockXY(solitaire, index1));
        Sprite.setLayer(components2.sprite, SublimeLayer[card2.direction == 'Up' ? 'CardUp' : 'CardDown']);
        Sprite.reset(components2.sprite, time, filmByID[getCardFilmID(card2)]);
    }
    for (const [index2, card3] of solitaire.waste.entries()){
        const components3 = ecs.componentsByRef.get(card3);
        Sprite.moveTo(components3.sprite, getWasteXY(filmByID, solitaire, index2));
        let animID1;
        ...
        Sprite.setLayer(components3.sprite, SublimeLayer[animID1 == 'CardDown' ? 'CardDown' : 'CardUp']);
        Sprite.reset(components3.sprite, time, filmByID[animID1]);
    }
}
function getStockXY(solitaire, indexY) {
    return I16XY(boardX, 16 + (solitaire.stock.length - 1 == indexY ? 0 : hiddenY));
}
function getWasteXY(filmByID, solitaire, index) {
    ...
    return I16XY(boardX + 8 + film.wh.x, 16 + betterIndex * 8);
}
function getFoundationCardXY(filmByID, suit) {
    ...
    const x = boardX + 48 + 2 * 8 + betterIndexX * (film.wh.x + 8);
    return I16XY(x, 16);
}
...
const VacantStockSystem = Immutable({
    ...
    skip (update) {
        return !!update.pickHandled || update.inputs.pick == null || !Input.activeTriggered(update.inputs.pick);
    },
    updateEnt (set, update) {
...
const CardSystem = Immutable({
    ...
    update (sets, update) {
        if (update.inputs.pick?.active == true) {
            if (Input.activeTriggered(update.inputs.pick) && (update.inputs.pick.buttons & Button.Primary) == Button.Primary) {
        ...
        if (update.solitaire.selected != null && Input.inactiveTriggered(update.inputs.pick)) {
            if (update.picked == null) {
...
(function(SublimeSolitaire1) {
    ...
    function onFrame(self, delta) {
        ...
        self.age = self.age + self.time - self.time % self.tick;
        ...
        if (Input.activeTriggered(self.inputPoller.inputs.pick) && ((self.inputPoller.inputs.pick?.buttons ?? 0) & Button.Secondary) == Button.Secondary) {
            if (!self.rendererStateMachine.isContextLost()) {
                self.rendererStateMachine.loseContext();
                setTimeout(()=>self.rendererStateMachine.restoreContext(), 5000);
            }
        }
    }
    SublimeSolitaire1.onFrame = onFrame;
})(SublimeSolitaire || (SublimeSolitaire = {}));
function findbestmatch(update) {
    const pointedCard = update.picked?.ents[0]?.components;
    let bestMatch;
...
function setPickRange(update, card) {
    ...
    const ents = selected.cards.map((card)=>{
        ...
        return {
            components,
            offset: I16XY.sub(I16XY(update.inputs.pick.xy), components.sprite.bounds.start)
        };
...
function moveToPick(update) {
    assertNonNull(update.inputs.pick);
    assertNonNull(update.picked);
    for(let i = 0; i < update.picked.ents.length; i++){
        I16Box.moveTo(update.picked.ents[i].components.sprite.bounds, I16XY.sub(I16XY(update.inputs.pick.xy), update.picked.ents[i].offset));
    }
}
function newBackground(factory) {
    return [
        {
            followCam: {
                fill: 'XY',
            ...
            sprite: factory.new('PaletteMid', 'Background', {
                layerSuborder: 'Start'
            })
        },
        {
            ...
            sprite: factory.new('Corner', 'Background', {
                flip: ''
            })
        },
        {
        ...
        },
        {
            ...
            sprite: factory.new('Grid', 'Background', {
                start: I16XY(1, 1),
                layerSuborder: 'Start',
                wrapX: I4(-1),
                wrapY: I4(-1)
...
function newPatienceTheDemon(factory) {
    return {
        followCam: {
            modulo: I16XY(8, 8),
            orientation: 'Northeast',
